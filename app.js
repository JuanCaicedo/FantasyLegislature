...
Elm.FantasySenate = Elm.FantasySenate || {};
Elm.FantasySenate.make = function (_elm) {
   "use strict";
   _elm.FantasySenate = _elm.FantasySenate || {};
   if (_elm.FantasySenate.values) return _elm.FantasySenate.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var legislatorsUrl = A2($Http.url,
   "https://congress.api.sunlightfoundation.com/legislators",
   _U.list([{ctor: "_Tuple2",_0: "apikey",_1: "d6ef0d61cbd241bc9d89109e4f70e128"},{ctor: "_Tuple2",_0: "per_page",_1: "all"}]));
   var Legislator = F2(function (a,b) {    return {firstName: a,lastName: b};});
   var legislatorsDecoder = function () {
      var legislator = A3($Json$Decode.object2,
      Legislator,
      A2($Json$Decode._op[":="],"first_name",$Json$Decode.string),
      A2($Json$Decode._op[":="],"last_name",$Json$Decode.string));
      return A2($Json$Decode._op[":="],"results",$Json$Decode.list(legislator));
   }();
   var initialModel = {selectedLegislators: _U.list([{firstName: "Juan",lastName: "Caicedo"},{firstName: "Carson",lastName: "Banov"}])
                      ,availableLegislators: _U.list([])};
   var update = F2(function (msg,model) {
      var _p0 = msg;
      if (_p0.ctor === "PopulateAvailableLegislators") {
            var _p1 = _p0._0;
            if (_p1.ctor === "Ok") {
                  return {ctor: "_Tuple2",_0: _U.update(model,{availableLegislators: _p1._0}),_1: $Effects.none};
               } else {
                  return {ctor: "_Tuple2",_0: _U.update(model,{availableLegislators: _U.list([])}),_1: $Effects.none};
               }
         } else {
            var _p3 = _p0._1;
            var legislatorFilter = function (currentLegislator) {
               return $Basics.not(_U.eq(currentLegislator.firstName,_p3.firstName)) && $Basics.not(_U.eq(currentLegislator.lastName,_p3.lastName));
            };
            var _p2 = _p0._0;
            if (_p2.ctor === "Available") {
                  return {ctor: "_Tuple2"
                         ,_0: _U.update(model,
                         {availableLegislators: A2($List._op["::"],_p3,model.availableLegislators)
                         ,selectedLegislators: A2($List.filter,legislatorFilter,model.selectedLegislators)})
                         ,_1: $Effects.none};
               } else {
                  return {ctor: "_Tuple2"
                         ,_0: _U.update(model,
                         {selectedLegislators: A2($List._op["::"],_p3,model.selectedLegislators)
                         ,availableLegislators: A2($List.filter,legislatorFilter,model.availableLegislators)})
                         ,_1: $Effects.none};
               }
         }
   });
   var legislatorsHeader = A2($Html.tr,
   _U.list([]),
   _U.list([A2($Html.th,_U.list([]),_U.list([$Html.text("First Name")])),A2($Html.th,_U.list([]),_U.list([$Html.text("Last Name")]))]));
   var Model = F2(function (a,b) {    return {selectedLegislators: a,availableLegislators: b};});
   var Selected = {ctor: "Selected"};
   var Available = {ctor: "Available"};
   var ToggleLegislator = F2(function (a,b) {    return {ctor: "ToggleLegislator",_0: a,_1: b};});
   var legislatorListItem = F3(function (address,destination,legislator) {
      return A2($Html.tr,
      _U.list([A2($Html$Events.onClick,address,A2(ToggleLegislator,destination,legislator))]),
      _U.list([A2($Html.td,_U.list([]),_U.list([$Html.text(legislator.firstName)])),A2($Html.td,_U.list([]),_U.list([$Html.text(legislator.lastName)]))]));
   });
   var legislatorsTable = F3(function (address,destination,legislators) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.table,
      _U.list([$Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: "table",_1: true}
                                                  ,{ctor: "_Tuple2",_0: "table-striped",_1: true}
                                                  ,{ctor: "_Tuple2",_0: "table-bordered",_1: true}]))]),
      _U.list([A2($Html.tbody,_U.list([]),A2($List._op["::"],legislatorsHeader,A2($List.map,A2(legislatorListItem,address,destination),legislators)))]))]));
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("container-fluid")]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("col-xs-6")]),
              _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Your team")])),A3(legislatorsTable,address,Available,model.selectedLegislators)]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("col-xs-6")]),
              _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Available")])),A3(legislatorsTable,address,Selected,model.availableLegislators)]))]));
   });
   var PopulateAvailableLegislators = function (a) {    return {ctor: "PopulateAvailableLegislators",_0: a};};
   var getLegislators = $Effects.task(A2($Task.map,PopulateAvailableLegislators,$Task.toResult(A2($Http.get,legislatorsDecoder,legislatorsUrl))));
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: initialModel,_1: getLegislators},update: update,view: view,inputs: _U.list([])});
   var main = app.html;
   var runner = Elm.Native.Task.make(_elm).performSignal("runner",app.tasks);
   return _elm.FantasySenate.values = {_op: _op
                                      ,PopulateAvailableLegislators: PopulateAvailableLegislators
                                      ,ToggleLegislator: ToggleLegislator
                                      ,Available: Available
                                      ,Selected: Selected
                                      ,Model: Model
                                      ,view: view
                                      ,legislatorsTable: legislatorsTable
                                      ,legislatorsHeader: legislatorsHeader
                                      ,legislatorListItem: legislatorListItem
                                      ,update: update
                                      ,initialModel: initialModel
                                      ,getLegislators: getLegislators
                                      ,Legislator: Legislator
                                      ,legislatorsDecoder: legislatorsDecoder
                                      ,legislatorsUrl: legislatorsUrl
                                      ,app: app
                                      ,main: main};
};
